...
for k in range(num_labels):
    ...
    c = [data_x_all_conv1[index] for index in [i for i,j in enumerate(data_y_all_conv1) if j == k][:firstItens]]    
    #Stack features horizontally
    d = np.hstack( (np.array(a), np.array(c)) )    
    
    b = [data_x_all_conv5[index] for index in [i for i,j in enumerate(data_y_all_conv5) if j == k][:firstItens]]
    #Stack features horizontally
    d = np.hstack( (np.array(a), np.array(b)) )
